...
*   文件名： RedisClient.cs
*
*   Copyright (c) 2015 chuxin Corporation. All rights reserved.
*****************************************************************/
using System;
...
namespace GenerSoft.IndApp.Redis
{
    public class RedisClient
    {
        ...
        /// <returns></returns>
        public bool StringSet<T>(string key, T obj)
        {
            key = AddSysCustomKey(key);
        ...
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="key">webconfig:appSettings->设置"ModelName" 实际保存的Key为"Model:"+Key</param>
        /// <returns></returns>
        ...
        /// <param name="expiry"></param>
        /// <returns></returns>
        public async Task<bool> StringSetAsync<T>(string key, T obj)
        {
            key = AddSysCustomKey(key);
            string json = ConvertJson(obj);
        ...
        public List<T> HashKeys<T>(string key)
        {
            ...
            return Do(db =>
            {
                ...
                {
                    result.Add(ConvertObj<T>(v.ToString()));
                }
                return result;
            });
        }
        /// <summary>
        /// 获取hashkey所有Redis HashEntity
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="key"></param>
        /// <returns></returns>
        public List<T> HashEntityAll<T>(string key)
        {
            key = AddSysCustomKey(key);
            List<T> result = new List<T>();
            return Do(db =>
        ...
        public void KeyMultiDel(string parm)
        {
            var keys = GetServer(CustomConfigParam.RedisConnectString.Split(',')[0]).Keys(pattern: parm).ToList();
            foreach (var item in keys) {
                Do(db => db.KeyDelete(item));
            }
        }
        /// <summary>
        /// 判断key是否存储
        ...
        /// <param name="expiry"></param>
        /// <returns></returns>
        public bool KeyExpire(string key)
        {
            key = AddSysCustomKey(key);
